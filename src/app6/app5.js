/*
    ...
    本质上,当Node启动一个文件或服务器后,Node实际上是运行在一个死循环中
    ...
    Node的单线程:所谓单线程,指的是Node的逻辑执行主线程是单线程的,即javascript代码运行所处的线程,这个单线程,因为javascript本身只能执行在单线程当中

    当我们在程序中引入了某个第三方模块时,那么整体的全部执行逻辑如下:

    Node->第三方模块->原生模块->原生模块内部的实现->c++模块->底层libuv(内置于操作系统)->线程池->线程->执行底层的IO操作(涉及到操作系统调用)

    当Node在执行过程中,会判断当前的操作系统类型
    ...
    对于底层的线池来说,它们却可以运行在多个核心上面,当然也可以同时运行,因此他们是完全可以利用多核的优势的.
